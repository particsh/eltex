...
static ssize_t device_write(struct file *filp, const char __user *buff, 
    ...
    short count = 0;
    short ind;          // индекс для обратного порядка
    char kernel_buf[100] = {0};

    if (len > 99)       // оставляем место для завершающего нуля
        len = 99;

    // копируем строку из пользовательского пространства в ядро
    ...
    // записываем в msg в обратном порядке
    for (ind = len - 1; ind >= 0; ind--) {
        msg[count++] = kernel_buf[ind];
    }
    // если нужно, добавляем завершающий нуль
    msg[count] = '\0';
